  ...
  const handleSave = useCallback(
    (values) => {
      ...
      if (values.new_passwd !== values.re_passwd) {
        setSaving(false);
        notify("new password does not match", {
          _: "new password does not match",
        });
      } else if (values.old_passwd === values.re_passwd) {
        setSaving(false);
        notify("The new password is the same as the old password", {
          _: "The new password is the same as the old password",
        });
      } else {
        auth
          .signInWithEmailAndPassword(currentUser.email, values.old_passwd)
          .then(async (userCredential) => {
            // Signed in
            const user = userCredential.user;
            await currentUser
              .updatePassword(values.new_passwd)
              .then(() => {
                setSaving(false);
              ...
              .catch((error) => {
                ...
                notify(
                  ...
                  "warning",
                  {
                    _: "A technical error occured while updating your password. Please try later.",
                  }
                );
              });
          })
          .catch((error) => {
            setSaving(false);
            notify("old password incorrect", {
              _: "old password incorrect",
            });
          });
      }
